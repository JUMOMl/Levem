...
class TicketPurchaseInterface:
    ...
    def view_available_events(self):
        ...
        for event in events:
            # ООП: Ассоциация - связь между
            # событиями и их билетами через ForeignKey.
            ticket_prices_list = self.session.query(Ticket.price).filter(
                Ticket.event_id == event.event_id
            ).all()
            price_list = (
                [float(price[0]) for price in ticket_prices_list]
                if ticket_prices_list
                else [0.0]
            )
            min_price = min(price_list) if price_list else "N/A"
            print(
                f"ID: {event.event_id}, Название: {event.title}, "
                f"Место: {event.location}," +
                f"Минимальная цена билета: {min_price}"
            )
        return events

    def purchase_ticket(self, user_id: int, event_id: int, price: int):
        """
        ...
        try:
            ...
            # ООП: Инкапсуляция - управление
            # добавлением и сохранением объекта в базе.
            ticket = Ticket(user_id=user_id, event_id=event_id, price=price)
            self.session.add(ticket)
            self.session.commit()
            logging.info(
                f"Пользователь с ID {user_id} успешно"
                + f"приобрел билет на мероприятие ID {event_id}."
            )
